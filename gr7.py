...
def get_info():
    ...
    #Idetify Outstanding orders that are not filled and close these in a few seconds
    ...
    #Circuit Breaker 1 : Identify when position grow across Prod A and B to a limit that will encroach on the 500 position limit
    ...
    # Circuit Breaker 2: Implement a price based circuit breaker in case the position went too negative in one direction

get_info()
"""
# retreives the best bid/ask price from exchange. If the best bid price for B is greater than the best ask for A, Buy from A and Sell to B

# Identify which Exchange to buy and sell at : Philips A more liquid & Philips B less liquid - So check for arbitrage & high Vols
# Idea is to buy off from exchange with greater Bid and sell to the other exchange 
# else stick to buying from more illiquid exchange and sell to less liquid exchange 
def whicheXchange():
